      ...
      <div className="mx-auto flex w-full max-w-[1328px] flex-col gap-16 lg:gap-20">
        <div className="flex flex-col gap-4 md:gap-6 lg:grid lg:grid-cols-4 lg:items-end lg:gap-4">
          ...
          <Reveal
            as="p"
            delay={0.14}
            className="max-w-[264px] text-[16px] font-medium leading-[1.44] tracking-[-0.01em] md:text-[18px]"
          >
            Start bright, slow down, or treat yourself whenever.
          </Reveal>
        </div>

        <div className="relative flex flex-col items-center gap-10 md:items-end md:gap-10">
          <div className="flex w-full min-w-0 flex-col overflow-hidden rounded-[24px] bg-[#1F3D38] p-[3px] text-[#FFFFFF]">
            <motion.div
              key={current.id}
              ...
              className="flex w-full min-w-0 flex-col gap-16 px-6 pt-6 pb-2"
            >
              ...
              <div className="flex w-full min-w-0 flex-col-reverse gap-6 lg:grid lg:grid-cols-12 lg:gap-4">
                <div className="flex min-w-0 overflow-hidden text-[#AAD0C8] [container-type:inline-size] lg:col-span-8">
                  <span className="font-heading text-[96px] leading-none tracking-[-0.01em] md:text-[clamp(6rem,22cqi,16.5rem)]">
                    {hh}
                  </span>
                  <span className="font-heading text-[96px] leading-none text-[#FFFFFF] md:text-[clamp(6rem,22cqi,16.5rem)]">
                    :
                  </span>
                  <span className="font-heading text-[96px] leading-none tracking-[-0.01em] md:text-[clamp(6rem,22cqi,16.5rem)]">
                    {mm}
            ...
            <div
              className="grid grid-cols-1 overflow-hidden rounded-b-[21px] gap-[3px] min-[768px]:grid-cols-2 lg:grid-cols-4"
              role="tablist"
              ...
              {timeSlots.map((slot) => {
                ...
                return (
                  <motion.button
                    ...
                    className={cn(
                      "flex min-w-0 cursor-pointer items-start justify-start gap-4 p-4 text-left transition-colors duration-300 sm:gap-6 sm:p-6",
                      on
          ...
          <img
            ...
            alt=""
            className="pointer-events-none absolute bottom-24 left-1/2 hidden w-20 -translate-x-1/2 rotate-[60deg] sm:block md:bottom-6 md:left-auto md:right-[222px] md:w-24 md:translate-x-0 md:rotate-[15deg]"
          />

          <div className="flex w-full flex-col md:w-auto">
            <MelloButton href="#menu" className="w-full max-[479px]:w-full md:w-auto">
              Explore the menu
            </MelloButton>
          </div>
        </div>
